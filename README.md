# 👋 Salut, je suis Yvan Kouemou

🎯 **Data Engineer | Data Analyst**

Passionné par la **data**, le **cloud** et la **visualisation**, j’aime concevoir des **pipelines robustes**, automatiser les flux de données et donner vie aux chiffres à travers des **dashboards percutants**.  
Mon objectif : transformer la donnée brute en **décisions stratégiques** grâce à des architectures modernes et des outils puissants.
---

## 🛠️ Stack technique

**💻 Langages & Outils**
> Python 🐍 · SQL 🧠 · Excel 📊 · Power BI 📈 · Looker 🔍  
> Talend ⚙️ · dbt 🧩 · GCP ☁️ · BigQuery 🏗️ · Docker 🐳 · Terraform 🪄 · Airflow 🔄  

---

## ⚡ Ce que j’aime faire

- 🔹 Créer des **pipelines ELT** performants et scalables  
- 🔹 Concevoir des **dashboards dynamiques** pour la prise de décision  
- 🔹 Intégrer des solutions **Cloud Data Warehouse**  
- 🔹 Mettre en place des architectures **DataOps & MLOps**  
- 🔹 Collaborer sur des projets alliant **ingénierie et storytelling data**  

---

## 🚀 Projets récents

| 🧩 Projet | ⚙️ Stack principale | 📚 Description |
|------------|--------------------|----------------|
| **BarcaRealStats** | Python · GCP · BigQuery · DBT · Looker · Docker | Analyse en temps réel des performances du Real Madrid et du FC Barcelone, avec automatisation IA via Gemini & X API |
| **Chelsea FC Vizathon** | Power BI · Excel · DAX | Dashboard analytique des performances physiques des joueurs du Chelsea FC, dans le cadre du Vizathon Contest |
| **Data Pipeline Cloud** | Talend · Airflow · GCP · BigQuery | Mise en place d’un pipeline complet d’intégration et transformation de données cloud |

---

## 📫 Mes coordonnées

💼 [**LinkedIn**](https://www.linkedin.com/in/yvan-kouemou-a421a6196/)  
✉️ [**Email**](yvankouemou@gmail.com)  
📂 [**Portfolio / Projets**](https://datascienceportfol.io/yvankouemou)

---

## 🧠 Citation favorite
> “Without data, you’re just another person with an opinion.” — *W. Edwards Deming*

---
⭐ *N’hésite pas à explorer mes projets ou à me contacter pour échanger autour de la data, du cloud et de la visualisation !*

